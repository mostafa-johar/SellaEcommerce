<template>
    <div class="flex md:gap-2 gap-1 justify-start p-3">
        <button
            ...
            <img src=" @/assets/imgs/cart-3.png" alt="cart" class="object-contain hidden md:block" loading="lazy">
            <span class="text-sm md:text-sm font-medium lg:text-white">
                أضف للسلة
            </span>
        </button>
        <div class="flex justify-center items-center border-2 border-gray-10 lg:p-3 p-2 " v-on:click="active = !active">
            <img :src="getImg(active ? likeHover : like) " alt="like" class="object-contain" loading="lazy">
        </div>
    </div>
</template>
