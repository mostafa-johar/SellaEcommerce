  ...
  <main class="AllProducts py-[30px] px-2">
    ...
    <article class="BreadCrumbs pb-3">
      <div class="container mx-auto">
        <ul class="flex gap-1 p-4">
          ...
          <li>/</li>
          <li>
            <router-link to="/AllProducts" class="text-gray-400"
    ...
    <div class="container mx-auto">
      ...
      <article class="flex gap-6">
        ...
        >
          <p class="text-gray-600 font-semibold text-sm py-4 px-4 border-b-2">
            فلترة المنتجات
          </p>
          <span
            ...
            v-on:click="widthToggle = !widthToggle"
            >x</span
          >
          <!-- 1 Categories -->

          <ul
            class="flex flex-col justify-start pt-4 px-4"
            v-for="(option, index) in options"
            :key="option.id"
          >
            <li class="mb-3" v-on:click="option.open = !option.open">
              <button class="flex justify-between gap-1 w-full">
                <h3 class="text-base text-gray-60 font-medium">
                  {{ option.title }}
                </h3>
                <img
                  src="@/assets/imgs/arrow_down.png"
                  alt="Arrow_Down"
                  :class="`object-contain transition-all ${
                    option.open ? '' : 'rotate-180'
                  }`"
                />
              </button>
            </li>
            ...
            <section
              :class="`${
                option.open ? 'h-auto' : 'h-0'
              } transition-all mb-6  overflow-hidden`"
            >
              <li
                class="flex justify-between gap-2 pb-2"
                v-for="(category, index) in option.categories"
                :key="category.id"
              >
                <label
                  :for="`${
                    category.title == 'الكل' ? option.title : category.title
                  }`"
                ...
                >
                  ...
                  />
                  <span
                    class="rounded-full w-3 h-3 bg-teal-400 hidden peer-checked:block"
                  ></span>
                </label>
              ...
              <span class="text-sm text-teal-400 cursor-pointer"
                >عرض المزيد</span
              >
            </section>
          ...
          <ul class="flex flex-col pt-4 px-4">
            ...
            <li class="flex py-6">
              <label
                for="minPrice"
                class="slide w-[50%] border-[1px] flex justify-center items-center"
              >
                <input
                  type="range"
                  min="1000"
            ...
            >
              ...
              >
                ...
                >
                  ...
                  />
                  <span
                    :class="`rounded-full w-3 h-3 ${color.color} hidden peer-checked:block`"
                  ></span>
                </label>
                <div class="flex justify-between flex-1">
                  <span class="flex gap-1">
                    <div
                      :class="`rounded-full ${color.color} w-5 h-5 `"
                      v-show="color.id != 1"
                    ></div>
        ...
        <section class="flex-1 basis-[80%] flex flex-col gap-8">
          ...
          <section
            class="flex justify-between items-center flex-wrap gap-2 border-[1px] shadow-sm p-4"
          >
            <div class="flex gap-2">
              <span
                class="border-[1px] rounded-md p-3 cursor-pointer"
                :class="netOne.active ? '' : 'bg-teal-400'"
                v-on:click="netOne.active = !netOne.active"
              >
                <img
                  :src="getImg(netOne.active ? netOne.imgOne : 'grid-3.png')"
                  alt="grid"
                  class="object-contain select-none"
                />
              </span>
              <span
                class="border-[1px] rounded-md bg-gray-10 p-3 cursor-pointer"
                :class="netOne.active ? 'bg-teal-400' : ''"
                v-on:click="netOne.active = !netOne.active"
              >
                <img
                  :src="getImg(netOne.active ? netOne.imgTwo : 'grid-4.png')"
                  alt="grid"
                  class="object-contain select-none"
                />
              </span>
            ...
            <div class="flex gap-2">
              <p class="text-sm text-gray-600">
                <span class="text-gray-400">ترتيب حسب</span> : من الاعلى الى
                الاقل
              </p>
              <img
                :src="getImg('arrow_down.png')"
                alt="arrow_down"
          ...
          <article class="products">
            ...
            >
              <!-- box -->
              <div
                class="grid rounded-md p-2"
                v-for="(product, index) in products"
                :key="product.id"
                v-show="index < Number(num)"
                :class="netOne.active ? 'grid-cols-1' : 'grid-cols-2'"
              >
                <img
                ...
                <div
                  class="flex flex-col gap-4 p-4 flex-1 justify-center basis-[60%]"
                >
                  <!-- 1 -->
            ...
            <span
              class="text-lg text-teal-400 cursor-pointer text-center block p-3"
              @click="increase"
              >عرض المزيد</span
            >
          </article>
